...
public class CharacterFeedBack {
	...
	public CharacterFeedBack() {
		bugs = new ArrayList<Tuple>();
		rights = new ArrayList<Tuple>();
		testCases = new ArrayList<TestCase>();
	}

	public void process(Chain workMachine, CaseRunner caseRunner,
	...
	public void character(Chain workMachine, CaseRunner caseRunner) {
		while (true) {
	...
	public int validateFaultyTuple(Tuple tuple, Chain workMachine,
	...
	public void runImportBugs(Chain workMachineOld,
			CorpTupleWithTestCase generate, CaseRunner caseRunner) {
		/**
			...
			if (suite.getAt(suite.getTestCaseNum() - 1).testDescription() == TestCase.PASSED) {
				for (int i = 0; i < suite.getTestCaseNum() - 1; i++) {
					...
					if (!isContainBugs(wrongCase)) {
						...
						Chain workMachine = new Chain(pool, generateNew);
